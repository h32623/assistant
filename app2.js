...
app.post('/reg_receiver', function(req, res){
    ...
    conn.query( sql_select, params[0], function (err, rows, fields) {
      // And done with the connection.
        ...
        if (err) {
         ...
         // if(rows.length > 1) 
          {
            console.log(rows + '/' + rows.length + '/' + rows[0].namesCount);
            res.send('환영합니다.' + params[2] + '님.');
      
            var sql_insert = 'INSERT INTO user(user_email, user_pw, user_name) VALUES (?,?,?);';
      
    //        connection.query(sql_insert, params, function (err, rows, fields) {
     //         if (err) {
     //           console.log(err);
      //        } else {
       //         console.log(rows);
  ...
  // conn.connect();
  // var sql_select = 'SELECT COUNT(*) FROM user WHERE user_email = ?;';
  // conn.query(sql_select, params[0], function (err, rows, fields) {
  //   if (err) {
  //     console.log(err);
  //   } 
